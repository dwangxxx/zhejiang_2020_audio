...
test_txt_2 = '/data/dengwang/audio/data/final_test_round2/test.txt'
# 2分类
...
""" for i in range(len(result_50)):
    print(result_50_arr[i][result_50[i]], result_50_sec[i][result_50_sec_max[i]]) """
...
""" for i in range(len(result_50)):
    if result_50_arr[i][result_50[i]] < 1.0:
        print(wavfiles_50[i], result_50_arr[i]) """
...
for i in range(len(result_50)):
    # 如果置信度不高，则将其辨别为伪造数据
    if result_50_arr[i][result_50[i]] < 0.2:
