...
class Data_Generator():
    def __init__(self, feat_path, train_csv, feat_dim, batch_size=32, alpha=0.2, shuffle=True, crop_length=400,
                 splitted_num=4):
        ...
        self.sample_num = len(open(train_csv, 'r').readlines())
        self.lock = threading.Lock()
        ...
        with self.lock:
            while True:
                ...
                for k in range(self.splitted_num):
                    ...
                    lines = indexes[s:e]
                    X_train, y_train = load_data_train(self.feat_path, self.train_csv, self.feat_dim, lines,
                                                               'logmel')
                    y_train = keras.utils.to_categorical(y_train, 52)
...
# 热重启 + 余弦退火优化学习率
class WarmRestart(keras.callbacks.Callback):
